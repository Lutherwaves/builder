...
It assumes `cctop`, `ctx-wire`, and `cctop/burn-proj.py` are on the host, and that
a status-line tap has written `~/.claude/cctop/usage.json` (see cctop's
usage-limits docs). Runtime state lives in `~/.claude/cctop/` and is not
committed.
